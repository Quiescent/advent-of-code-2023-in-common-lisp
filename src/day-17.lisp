...
(defun cheapest-2 (grid)
  (bind ((shortest (make-hash-table :test #'equal))
         ...
         (destination (complex (length (aref grid 0)) (length grid))))
    (iter
      (while (not (empty? remaining)))
      (for (c cost dir) = (first remaining))
      ;; (format t "(list c cost): ~a~%" (list c cost))
      (when (= c destination)
      ...
      (iter
        ...
        (for new-cost = (+ cost
                           (iter
                             (for j from 1 to i)
                             (summing (grid-at grid (+ c (* i new-dir)))))))
        (when (<= (gethash new-c shortest most-positive-fixnum) new-cost)
          (next-iteration))
        (setf remaining            (with-first remaining (list new-c new-cost new-dir))
